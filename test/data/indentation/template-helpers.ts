...
// Helper functions for indentation template
// Handles cursor position highlighting and shows where indentation occurs
...
export function highlightLedgerSyntax(
  ...
  return lines
    ...
    .map((line, lineIndex) => {
      // Add visual newline indicator to all lines except the last one
      if (lineIndex < lines.length - 1) {
        return line + '<span class="newline">↵</span>';
      }
      return line;
    })
    .join("\n");
...
function applySyntaxHighlighting(line: string): string {
  ...
  // Highlight leading spaces for indentation visualization
  line = line.replace(/^( +)/g, '<span class="indent">$1</span>');
