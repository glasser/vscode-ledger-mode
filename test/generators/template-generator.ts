...
const ANSI_COLORS_CSS = fs.readFileSync(path.join(cssAssetsDir, 'ansi-colors.css'), 'utf8');
const ERROR_FORMATTING_CSS = fs.readFileSync(path.join(cssAssetsDir, 'error-formatting.css'), 'utf8');
...
function loadTemplateHelpers(testDataDir: string): void {
  ...
  if (fs.existsSync(helpersPath)) {
    try {
      ...
      console.warn(`Could not load template helpers from ${helpersPath}:`, error);
    }
  }
}
...
function loadTestCases(testDataDir: string): TestCase[] {
  ...
  try {
    ...
    const testNames = [...new Set(
      files.filter(f => f.endsWith('-input.txt') || f.endsWith('-expected.html'))
    ...
    const jsonFiles = files.filter(f => f.endsWith('.json') && !f.includes('data.test'));
    ...
    const ledgerFiles = files.filter(f => f.endsWith('.ledger') && !f.includes('data.test'));
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          if (fs.existsSync(testCasesDir)) {
            ...
            const testCaseEntries = fs.readdirSync(testCasesDir, { withFileTypes: true });
            for (const testCaseEntry of testCaseEntries) {
              if (testCaseEntry.isDirectory()) {
                const testCasePath = path.join(testCasesDir, testCaseEntry.name);
    ...
    console.warn(`Warning: Could not load test cases from ${testDataDir}:`, error);
  }
...
if (require.main === module) {
  ...
  const directories = fs.readdirSync(testDataDir, { withFileTypes: true })
  ...
  for (const dir of directories) {
    ...
    if (templateFile) {
      try {
        ...
        console.log(`Generated ${result.outputFile} with ${result.testCases} test cases`);
      } catch (error) {
