...
2024-01-02 Store B
    ...
    assert.deepStrictEqual(payees1, payees2, "Cached results should be identical");
    assert.strictEqual(payees1.size, 2, "Should have 2 payees");
    
    // Cache should be much faster (at least 2x, often 10x+)
    assert.ok(time2 < time1 / 2, `Cache should be much faster (parse: ${time1.toFixed(3)}ms, cache: ${time2.toFixed(3)}ms)`);
    ...
    assert.ok(time3 > time2 * 2, `Cache should have expired (cached: ${time2.toFixed(3)}ms, expired: ${time3.toFixed(3)}ms)`);
    assert.deepStrictEqual(payees1, payees3, "Results should still be identical after cache expiry");
  ...
  test("Should share parsed transactions between payees and accounts", async () => {
    ...
    assert.ok(time2 < time1 / 2, `Accounts call should reuse cached data (payees: ${time1.toFixed(3)}ms, accounts: ${time2.toFixed(3)}ms)`);
  ...
  test("Should demonstrate performance with simulated large file", async function() {
    ...
    console.log(`  Testing with ${content.split('\n').length} lines, 5000 transactions`);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (i === 0) {
        console.log(`  First call (parse): ${time.toFixed(3)}ms (${payees.size} payees)`);
      } else if (i === 1) {
    ...
    const avgCacheTime = times.slice(1).reduce((a, b) => a + b, 0) / (times.length - 1);
    
    console.log(`  Average cache time: ${avgCacheTime.toFixed(3)}ms`);
    console.log(`  Performance improvement: ${(parseTime / avgCacheTime).toFixed(1)}x faster`);
    
    // Cache should be significantly faster
    assert.ok(avgCacheTime < parseTime / 5, `Cache should be at least 5x faster (parse: ${parseTime.toFixed(3)}ms, cache: ${avgCacheTime.toFixed(3)}ms)`);
  });
});